...
app.get('/posts/:id', (req, res) => {
  ...
  // the error response comes from server.js
  // lines 20 through 22 from the app.get('/')
  // route
  res.send(posts[id]);
});
...
app.delete('/posts/:id', (req, res) => {
  const id = parseInt(req.params.id);
  const searchIndex = posts.findIndex((post) => post.id === id);
  // findIndex() returns -1 if it can't find anything
  // so by stating searchIndex > -1 we're saying IF
  // this post is found then delete it
  // else
  // state 'nothing found'
  if (searchIndex > -1) {
